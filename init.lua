	...
	callback = function()
		...
		if vim.fn.executable("phpantom_lsp") == 0 and vim.fn.executable("cargo") ~= 0 then
			...
			vim.fn.jobstart({ "cargo", "install", "phpantom_lsp", "--locked" }, {
				on_exit = function(_, code)
					vim.notify(
						code == 0 and "PHPantom installed!" or "PHPantom install failed — see :messages",
		...
		if vim.fn.executable("rustfmt") == 0 and vim.fn.executable("rustup") ~= 0 then
			...
			vim.fn.jobstart({ "rustup", "component", "add", "rustfmt" }, {
				on_exit = function(_, code)
					vim.notify(
						code == 0 and "rustfmt installed!" or "rustfmt install failed — see :messages",
						code == 0 and vim.log.levels.INFO or vim.log.levels.ERROR
					)
				end,
			})
		...
		if vim.fn.executable("mago") == 0 then
			...
			local ok = vim.fn.system({
				"bash",
				"-c",
				'curl --proto "=https" --tlsv1.2 -sSf https://carthage.software/mago.sh | bash -s -- --install-dir="$HOME/.local/bin"',
			})
		...
		if vim.fn.executable("opencode") == 0 then
			...
			vim.fn.jobstart({ "bash", "-c", "curl -fsSL https://opencode.ai/install | bash" }, {
				on_exit = function(_, code)
					vim.notify(
						code == 0 and "opencode CLI installed!" or "opencode install failed — see :messages",
						code == 0 and vim.log.levels.INFO or vim.log.levels.ERROR
					)
				end,
			})
...
require("lazy").setup({
	...
	{ -- Useful plugin to show you pending keybinds.
		...
		opts = {
			...
			-- Register key groups so which-key shows nice labels
			spec = {
	...
	{ -- You can easily change to a different colorscheme.
		...
		config = function()
			...
			-- Theme toggle: press <leader>u to switch between night (dark) and day (light)
